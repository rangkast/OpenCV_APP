...
def remake_3d_point(camera_k_0, camera_k_1, RT_0, RT_1, BLOB_0, BLOB_1):
    ...
    return get_points
def mapping_id_blob(Tracking_ANCHOR, blob_centers, tcx, tcy):
    ...
    if same_position_blob:
        ...
        if dist_tracker_blob < 1:  # assuming 1 as threshold distance
            ...
            led_candidates = [(idx, blob_centers[idx][0],
                              ...
                              for idx in range(len(blob_centers)) if blob_centers[idx][0] > tcx]
            led_candidates.sort(key=lambda x: x[2])
            return led_candidates
        else:
            print('same blob not found')
            return ERROR
...
def gathering_data(bboxes):
    ...
    cv2.destroyAllWindows()
def select_keypoints_and_labels(image):
    keypoints = []
    labels = []

    def mouse_callback(event, x, y, flags, params):
        if event == cv2.EVENT_LBUTTONUP:
            # Enter the label for the new keypoint
            label = input("Enter the label for this keypoint: ")
    ...
    while True:
        for i, (x, y) in enumerate(keypoints):
            ...
            cv2.putText(image, str(labels[i]), (x+10, y+10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
def detect_and_label_led(image, prev_led_blobs=None, prev_descriptors=None, initial_keypoints=None, initial_labels=None, frame_cnt=0):
    # LED 검출
    led_blobs = detect_led_lights(image)
    led_blobs.sort(key=lambda blob: blob[0])

    # ORB 디스크립터 초기화
    orb = cv2.ORB_create()

    # 디스크립터 계산
    keypoints = [cv2.KeyPoint(x + w / 2, y + h / 2, max(w, h)) for x, y, w, h in led_blobs]
    try:
        keypoints, descriptors = orb.compute(image, keypoints)
    except Exception as e:
        print("Image is None or keypoints are empty:", e)
        return None, None

    if prev_led_blobs is None or prev_descriptors is None:
        labels = list(range(1, len(led_blobs) + 1))
        if initial_keypoints is not None and frame_cnt == 0:
            labels = [0]*len(keypoints)
            for i_kp, i_label in zip(initial_keypoints, initial_labels):
                dists = [np.linalg.norm(np.array((kp.pt[0]-i_kp[0], kp.pt[1]-i_kp[1]))) for kp in keypoints]
                labels[dists.index(min(dists))] = i_label
    else:
        if prev_descriptors is not None and descriptors is not None:
            if prev_descriptors.shape[1] == descriptors.shape[1]:
                ...
                labels = [0] * len(led_blobs)
                for match in matches:
                    if match.trainIdx < len(labels) and match.queryIdx < len(prev_led_blobs):
                        labels[match.trainIdx] = prev_led_blobs[match.queryIdx][4]  # 이전 라벨을 가져옴
            else:
                print("Descriptors shape mismatch.")
                print(f"prev_descriptors.shape: {prev_descriptors.shape}, type: {type(prev_descriptors)}")
                print(f"descriptors.shape: {descriptors.shape}, type: {type(descriptors)}")
                labels = [0] * len(led_blobs)
        else:
            print("One or both descriptors are None.")
            labels = [0] * len(led_blobs)

    # 라벨 추가
    led_blobs = [(*blob, label) for blob, label in zip(led_blobs, labels)]

    return led_blobs, descriptors
...
def ORB_Tracking():
    ...
    while frame_cnt < len(image_files):
        ...
        if frame_cnt == 0:
            keypoints, labels = select_keypoints_and_labels(frame_0)

        draw_frame = frame_0.copy()
        _, frame_0 = cv2.threshold(cv2.cvtColor(frame_0, cv2.COLOR_BGR2GRAY), CV_MIN_THRESHOLD, CV_MAX_THRESHOLD,
                                   cv2.THRESH_TOZERO)
        cv2.putText(draw_frame, os.path.basename(image_files[frame_cnt]), (draw_frame.shape[1] - 300, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)

        # LED 검출 및 라벨링
        led_blobs, descriptors = detect_and_label_led(frame_0, prev_led_blobs, prev_descriptors, keypoints, labels, frame_cnt)

        # 라벨링된 LED 표시
        if led_blobs is not None:
            for x, y, w, h, label in led_blobs:
                cv2.rectangle(draw_frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.putText(draw_frame, str(label), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        prev_led_blobs = led_blobs if led_blobs is not None else prev_led_blobs
        prev_descriptors = descriptors if descriptors is not None else prev_descriptors
